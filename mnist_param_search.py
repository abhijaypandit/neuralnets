...
if __name__ == "__main__":
    ...
    search = sklearn.model_selection.RandomizedSearchCV(model, search_space, n_iter=10, cv=3)

    # Random search for params
    print("Performing random search for parameters...")
    start = time.time()
    train_model(search, X_train, y_train, h_params, callbacks)
    end = time.time()
    print("Search complete. Time taken = {:.2f}s".format(end-start))
